 ...
 *
 *
 *line 1237:1772 command implements
 *
 * line:1787  main program 
 *
 *
 *
 */
int main(int argc, char **argv) {
    initServerConfig(); // 初始化server 变量
    initServer();   //建立listen socket , eventloop 
    if (argc == 2) {
        ResetServerSaveParams(); // param  save 900 1   900 second change one .save 
        loadServerConfig(argv[1]); //read configure file   ,空格切分处理
        redisLog(REDIS_NOTICE,"Configuration loaded"); // 写入到 server.logfile 中 level content
    } else if (argc > 2) {
